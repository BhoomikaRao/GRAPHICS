...
void drawship()
{
	...
	else if(ship.explode)
	{
		...
		{
			gamestate=over;
			glutPostRedisplay(); //display the current window again
		}
	}
	FireBulletsIfShot();
...
void drawbullet()
{
	...
	for(i=0;i<NumberOfBulletsPerFrame;i++)
	{
		...
		if(b[i].y > 500)
		{
			b[i].reinit();	//stop firing beyond the plane
...
int main(int argc, char** argv)
{
	...
	glutPassiveMotionFunc(move);	//movement of the ship
	glutKeyboardFunc(keyboard);		//to play the game
	glutSetCursor(GLUT_CURSOR_NONE);	//cursor on the game window
